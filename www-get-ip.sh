...
get_ip(){
  ...
  { eval timeout 2 wget ${1}; printf '\n';} 2>/dev/null | while IFS= read -r S; do
    S=$(printf '%s' "${S}" | sed "s/&#46;/./g" | grep -om1 '\(>\|^\|[[:blank:]]\|["$q"]\)\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\.\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\.\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\.\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\(<\|["$q"]\|[[:blank:]]\|$\)'
    )
    [ -n "${S}" ] && { set -- ${S}; printf '%s\n' "${1}"; break ;}
  done
}
